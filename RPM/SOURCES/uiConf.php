...
//		if ( $sub_type == self::FILE_SYNC_UICONF_SUB_TYPE_DATA )
	...
	public function generateFilePathArr($sub_type, $version = null, $externalPath = false )
	{
		...
		if ($externalPath)
		{
			$file_root = myCloudUtils::getPartnerSharedStorageBaseDir($this->getPartnerId());
	...
	private function setConfFileBySuffix($v, $suffix = null)
	{
	...
	{
		$this->setConfFileBySuffix($v, null);
	}
	...
	{
		$this->setConfFileBySuffix($v,self::FILE_NAME_FEATURES);
	}
	...
	{
		$this->setConfFileBySuffix($v,self::FILE_NAME_CONFIG);
	}
...
//		$cloned->setNew(true);
		...
		foreach (self::$validSubTypes as $subType) 
		{
			...
			$cloned->setConfFileBySuffix($content, $suffix);			
		}
