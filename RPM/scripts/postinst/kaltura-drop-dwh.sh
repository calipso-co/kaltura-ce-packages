...
for i in `mysql -h $DWH_HOST -p$PASSWD kalturadw -N -e "show tables"`;do mysql -h$DWH_HOST  -p$PASSWD kalturadw -e "drop view $i";done
...
for i in `mysql -h $DWH_HOST -p$PASSWD kalturalog -N -e "show tables"`;do mysql -h$DWH_HOST  -p$PASSWD kalturalog -e "drop view $i";done
